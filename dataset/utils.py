...
def untransform_prostate(img, lt):
...
def decode_seg_map_sequence(label_masks, dataset='cityscapes_19'):
    rgb_masks = []
...
def decode_seg_map_sequence_numpy(label_masks, dataset='cityscapes_19'):
    rgb_masks = []
    for label_mask in label_masks:
        rgb_mask = decode_segmap_0(label_mask, dataset)
        rgb_masks.append(rgb_mask)
    rgb_masks = np.array(rgb_masks)
    return rgb_masks

def decode_segmap_0(label_mask, dataset, plot=False):
    """Decode segmentation class labels into a color image(bk: (0, 0, 0))
    Args:
    ...
    elif dataset == 'cityscapes_16':
        n_classes = 16
        label_colours = get_cityscapes_labels_16()
    else:
