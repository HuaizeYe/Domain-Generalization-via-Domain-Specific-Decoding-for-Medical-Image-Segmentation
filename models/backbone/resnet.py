...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, zero_init_residual=False, groups=1,
        ...
        self.channels = [self.inplanes, 64 * block.expansion, 128 * block.expansion,
...
def _resnet(arch, block, layers, pretrained, **kwargs):
    ...
    if pretrained:
        state_dict = torch.load("/data/ziqi/pretrained_models/%s-official.pth" % arch)
        model.load_state_dict(state_dict, strict=False)
    return model

'''unet(output_stride = 32)
'''
...
'''deeplabv3p, pspnet(output_stride = 8)
def resnext50_32x4d(pretrained=False, **kwargs):
    kwargs['groups'] = 32
    kwargs['width_per_group'] = 4
    return _resnet('resnext50_32x4d', Bottleneck, [3, 4, 6, 3],
                   pretrained, replace_stride_with_dilation=[False, True, True], **kwargs)
...
def resnet152(pretrained=False):
    return _resnet('resnet152', Bottleneck, [3, 8, 36, 3], pretrained,
                   replace_stride_with_dilation=[False, True, True])
'''
model = resnet50().cuda(device=4)
data = torch.randn(1, 3, 256, 256).cuda(device=4)
b = model(data)
print(b[4].shape)
'''