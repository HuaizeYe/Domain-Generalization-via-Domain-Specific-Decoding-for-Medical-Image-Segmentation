...
class Unet2D(nn.Module):
    def __init__(self, c=3, n=16, norm='bn', num_classes=2, dropout_rate=0.5):
    ...
    def forward(self, x, weights=None):
        if weights == None:
            ...
            x1 = self.convd1(x, weights=weights, layer_idx='module.convd1')
            x2 = self.convd2(x1, weights=weights, layer_idx='module.convd2')
            x3 = self.convd3(x2, weights=weights, layer_idx='module.convd3')
            x4 = self.convd4(x3, weights=weights, layer_idx='module.convd4')
            x5 = self.convd5(x4, weights=weights, layer_idx='module.convd5')

            y4 = self.convu4(x5, x4, weights=weights, layer_idx='module.convu4')
            y3 = self.convu3(y4, x3, weights=weights, layer_idx='module.convu3')
            y2 = self.convu2(y3, x2, weights=weights, layer_idx='module.convu2')
            y1 = self.convu1(y2, x1, weights=weights, layer_idx='module.convu1')
        ...
        return y1_pred, predictions, x5


class ConvU2(nn.Module):
    def __init__(self, planes, norm='bn', first=False):
        super(ConvU2, self).__init__()
...
class UnetDecoder(nn.Module):
    def __init__(self, c=3, n=16, norm='bn', num_classes=3, dropout_rate=0.5):
        super(UnetDecoder, self).__init__()

        self.dropout_rate = dropout_rate

        self.convu4 = ConvU2(16 * n, norm, first=True)
        self.convu3 = ConvU2(8 * n, norm)
        self.convu2 = ConvU2(4 * n, norm)
        self.convu1 = ConvU2(2 * n, norm)
    ...
    def forward(self, x):
        ...
        y1 = self.convu1(y2)
        y1_pred = conv2d(y1, self.seg1.weight, self.seg1.bias, kernel_size=None, stride=1, padding=0)
        predictions = torch.tanh(y1_pred)
        # predictions = torch.sigmoid(y1_pred)
        return y1_pred, predictions