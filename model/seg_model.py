...
class ResNetMulti(nn.Module):
    def __init__(self, block, layers, in_dim, num_classes):
        self.inplanes = 64
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilation=1):
        ...
        #     i.requires_grad = False
        layers = []
        layers.append(block(self.inplanes, planes, stride, dilation=dilation, downsample=downsample))    #bottleneck
...
def DeeplabMulti(pretrained,layers, in_dim=1, num_classes=5):
    model = ResNetMulti(Bottleneck, layers, in_dim, num_classes)
    if pretrained:
        print("Loading from ", RESTORE_FROM)
        saved_state_dict = model_zoo.load_url(RESTORE_FROM)
        new_params = model.state_dict().copy()
        for i in saved_state_dict:
            i_parts = i.split('.')
            if num_classes == 5 or not (i_parts[1] == 'layer5' or i_parts[1] == 'conv1' and in_dim == 1):
                print(i)
